...
function showComplex(url, interactiveResID, curInteractiveResID, chain) {
	...
	// Here, it is a little awkward to have the ":", which is inconsistent with the residue selections 
	// Tried everything to make the parsing correct, but it insists to have a pre-set ":" in here to make it work
	// Did not figure out why, to get it to work, I'm keeping it this way
	
		chainScript[i] = 'select MODEL=1.1 and :' + chain[i][0] + '; wireframe off; cartoon; cpk off; color \"' + chain[i][2] + '\";';
// 		document.write(chainScript[i]);
	}	
